...
export default function ContributionForm({initialSection=""}:{initialSection?:string}){
 ...
 async function submit(e:FormEvent<HTMLFormElement>){e.preventDefault();setState("sending");const form=e.currentTarget,r=await fetch("/api/submissions",{method:"POST",body:new FormData(form)}),d=await r.json().catch(()=>({}));if(r.ok){form.reset();setSection("");setState("done")}else{setMessage(d.error??"提交失败，请重试。");setState("error")}}
 return <section className="form-shell">{state==="done"?<div className="success"><h2>✓ 已送到老师的审核箱</h2><p>老师审核后，内容会进入你选择的组别或页面位置。</p><button onClick={()=>setState("idle")}>继续提交</button></div>:<form onSubmit={submit}>
  ...
  <h2><span>02</span> 填写文字</h2><label>提交人 / 小组署名<input required maxLength={40} name="submitterName" placeholder="例如：一组 / 王同学"/></label><label>{max>0?"姓名或显示名称":"项目标题"}<input required maxLength={80} name="title" placeholder={max>0?"将显示在人物卡片上":"用一句话写出项目名称"}/></label><label>内容说明<textarea required maxLength={600} rows={6} name="body" placeholder="项目问题、过程、成果说明，或人物介绍、班级故事……"/></label>
  <h2><span>03</span> 上传图片</h2><label className="upload"><input type="file" name="image" accept="image/jpeg,image/png,image/webp"/><strong>JPG / PNG / WebP，不超过 5MB</strong></label>{section==="future-message"&&<label>可见范围<select name="visibility" value={visibility} onChange={e=>setVisibility(e.target.value)}><option value="public">审核后公开展示</option><option value="private">仅教师可见，不公开</option></select></label>}<input className="bot" name="website" tabIndex={-1}/><label className="agree"><input type="checkbox" name="consent" value="yes" required/>我确认文字和图片可以按上述可见范围用于本班课程网站。</label>{state==="error"&&<p className="error">{message}</p>}<button className="submit" disabled={state==="sending"}>{state==="sending"?"正在提交…":"提交给老师审核 →"}</button>
 </form>}</section>
}
